...
if $flgUpdate -eq true ;
        then
         echo "Docker will be updated..."
          ...
          docker rm $dockerId
          if ! docker run -d -p 7075:7075/udp -p 7075:7075 -p [::1]:7076:7076 -v /home/nanonode/:/root --name $dockerName --restart unless-stopped $dockerImage ; then
			   docker run -d -p 7075:7075/udp -p 7075:7075 -p 127.0.0.1:7076:7076 -v /home/nanonode/:/root --name $dockerName --restart unless-stopped $dockerImage
		  fi
		  echo "$currentDate Docker Container ($dockerName) updated with Image ($dockerImage)" >> /var/log/cron_nano_docker_updates.txt
else
